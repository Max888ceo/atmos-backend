...
// ============================================
app.get('/', (req, res) => {
  res.json({ status: 'ATMOS BACKEND ONLINE', version: '2.3.0' });
});
...
// Source: FMP API - House & Senate disclosures
// ============================================
app.get('/api/politicians', async (req, res) => {
  try {
    ...
    const endpoint = chamber === 'senate'
      ? `https://financialmodelingprep.com/stable/senate-latest-trading?apikey=${FMP_KEY}`
      : `https://financialmodelingprep.com/stable/house-latest-trading?apikey=${FMP_KEY}`;
    ...
    const trades = (Array.isArray(data) ? data : data.data || []).slice(0, 50).map(t => ({
      ...
      amount:        t.amount || t.transactionAmount || '?',
...
app.listen(PORT, () => {
  console.log(`ATMOS BACKEND v2.3.0 running on port ${PORT}`);
});
