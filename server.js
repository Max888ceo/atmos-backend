...
// ============================================
app.get('/', (req, res) => {
  res.json({ status: 'ATMOS BACKEND ONLINE', version: '2.6.0' });
});
...
app.get('/api/news', async (req, res) => {
  try {
    const query  = req.query.q || 'markets economy central bank inflation';
...
app.listen(PORT, () => {
  console.log(`ATMOS BACKEND v2.6.0 running on port ${PORT}`);
});
