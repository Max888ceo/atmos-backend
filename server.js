...
// ============================================
app.get('/', (req, res) => {
  res.json({ status: 'ATMOS BACKEND ONLINE', version: '2.1.0' });
});

// ============================================
// COT DATA ENDPOINT
// Source: CFTC Legacy Futures Only (6dca-aqww)
// This dataset has clean noncomm/comm fields for all major markets
// ============================================
...
function calcSentiment(ncLong, ncShort, prevNcLong, prevNcShort) {
  ...
  if (netNow > 0 && change < 0)  return 'BULLISH'; // still net long, fading
  if (netNow < 0 && change <= 0) return 'BEARISH';
  if (netNow < 0 && change > 0)  return 'BEARISH'; // still net short, fading
  return 'NEUTRAL';
}
...
app.get('/api/cot', async (req, res) => {
  try {
    ...
    // Legacy Futures Only dataset — has noncomm_positions_long_all etc.
    ...
    // Filter out micro contracts — prefer full-size
...
app.listen(PORT, () => {
  console.log(`ATMOS BACKEND v2.1.0 running on port ${PORT}`);
});
